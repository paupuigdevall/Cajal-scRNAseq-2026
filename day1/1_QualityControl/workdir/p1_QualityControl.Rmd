...
author: |
  Original Authors: Åsa Björklund, Paulo Czarnewski, Susanne Reinsbach, Roy Francis  
...
The data used in the first six tutorials is 10x Genomics single-cell RNA-seq data from peripheral blood mononuclear cells (PBMCs) collected from COVID-19 patients and healthy controls, described in the Science paper *“Immunophenotyping of COVID-19 and influenza highlights the role of type I interferons in development of severe COVID-19”*.

A peripheral blood mononuclear cell (PBMC) is any peripheral blood cell with a round nucleus. These cells include lymphocytes (T cells, B cells, NK cells), monocytes, and dendritic cells. In contrast, erythrocytes and platelets lack nuclei, and granulocytes (neutrophils, basophils, and eosinophils) have multi-lobed nuclei.

The original dataset was downloaded from GEO accession **GSE149689**. For these tutorials, we selected four severe COVID-19 donors and four healthy controls from that dataset. Each donor was downsampled to **1500 cells** per individual to reduce runtime during the labs. The script used for sample selection and downsampling is available here:

<https://nbisweden.github.io/workshop-scRNAseq/other/data.html>
...
> [NBIS single-cell RNA-seq course](https://nbisweden.github.io/workshop-scRNAseq/labs/seurat/seurat_01_qc.html).
...
```{r}
getwd()
#Modified paths for Noppe environment
path_covid <- "../data/covid/raw"
path_results <- "../data/covid/results"

```

In the Noppe environment, the example data files are already present under `./data/covid/raw`, so you can skip downloading. The original download code is included below but commented out.
...
```{r qc-vln, fig.height=8, fig.width=15}
...
```{r qc-scatter, fig.height=8, fig.width=15}
...
```{r top-genes, fig.height=8, fig.width=15}
...
```{r qc-vln2, fig.height=8, fig.width=15}
...
In Noppe, the precomputed annotation file is already present under `./data/covid/results`. The original download code is included below but commented out.
...
path_results <- "/shared/projects/tp_2616_fnom_183960/Puigdevall_P/Cajal-scRNAseq-2026/day1/1_QualityControl/data/covid/results"
genes_file <- file.path(path_results, "genes_table.csv")
...
```{r cc-scatter, fig.height=7, fig.width=7}
...
```{r doublet-plot, fig.height=4, fig.width=10}
...
In Noppe, this QC-filtered object is already provided in `data/covid/results/seurat_covid_qc.rds`. If you are running the tutorial elsewhere and want to write the file yourself, uncomment the code below.
